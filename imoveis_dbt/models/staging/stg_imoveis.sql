...
zone_correction AS (
	SELECT
		bairro,
		CASE
		WHEN zona = 'zona-sul' THEN 'Zona Sul'
		WHEN zona = 'zona-norte' THEN 'Zona Norte'
		WHEN zona = 'zona-oeste' THEN 'Zona Oeste'
		WHEN zona = 'zona-central' THEN 'Zona Central'
		ELSE 'Outros'
END zona_corrigida
FROM
	(
...
CASE
	...
	FROM
		deduplicated d
		LEFT JOIN zone_correction z ON d.bairro = z.bairro
	WHERE
...
filtered AS (
	...
	FROM
		cleaned c
		JOIN percentiles p ON c.bairro = p.bairro
		AND c.transacao = p.transacao
	WHERE
		c.preco BETWEEN p.p01_preco
		AND p.p99_preco
		AND c.area BETWEEN p.p01_area
		AND p.p99_area
)
SELECT
	*
